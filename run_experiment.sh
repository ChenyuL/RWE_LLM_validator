...
# Read the list of papers using Python
...
for PAPER in $PAPERS; do
    ...
    if [ -f "$PAPER_PATH" ]; then
        ...
        if python run_validation_with_retry.py --script test_record_validation_openai_claude_fixed.py --mode extractor --prompts "$PROMPT_FILE" --paper "$PAPER_PATH" --checklist "$CHECKLIST_NAME" --retries 3 --delay 30; then
        ...
        find output -name "*_${PAPER_ID}_${CHECKLIST_NAME}.json" -newer "$OUTPUT_DIR/fixed_sampled_papers.json" -exec mv {} "$OUTPUT_DIR/" \;
...
for PAPER in $PAPERS; do
    ...
    if [ -f "$PAPER_PATH" ]; then
        ...
        # Run the validation with error handling and retry capability
        echo "Running validation for $PAPER_ID..."
        if python run_validation_with_retry.py --script test_record_validation_claude_openai_fixed.py --mode extractor --prompts "$PROMPT_FILE" --paper "$PAPER_PATH" --checklist "$CHECKLIST_NAME" --retries 3 --delay 30; then
            echo "Validation completed successfully for $PAPER_ID"
        else
            echo "WARNING: Validation failed for $PAPER_ID, but continuing with next paper"
        fi
