#!/usr/bin/env python
# test_record_validation_openai_claude.py
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("record_test_openai_claude.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("record_test_openai_claude")
...
class OpenAIClaudeValidationFramework:
    ...
    def _save_results(self, paper_path, guideline_info, paper_info, validation_results, final_report):
        ...
        reasoner_filename = f"{timestamp}_openai_reasoner_{paper_name}_prompts.json"
        ...
        log_filename = f"{timestamp}_openai_reasoner_{paper_name}_process_log.txt"
        ...
        extractor_filename = f"{timestamp}_openai-gpt4o_extractor_{paper_name}_extraction.json"
        ...
        validator_filename = f"{timestamp}_claude-sonnet_validator_{paper_name}_validation.json"
        with open(os.path.join(OUTPUT_PATH, validator_filename), "w") as f:
            json.dump(validation_results, f, indent=2)
        
        # Save final report
        report_filename = f"{timestamp}_openai_claude_report_{paper_name}.json"
