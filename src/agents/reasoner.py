...
class Reasoner:
    ...
    def extract_guideline_items(self, guideline_texts: List[str]) -> List[Dict[str, Any]]:
        ...
        """
        self.logger.info("Extracting guideline items from texts")
        
        # Combine all guideline texts
        ...
        {text}
        """.format(text=combined_text[:100000])  # Limit text length to avoid token limits
        
        # Call LLM to extract guideline items
        result = self._call_llm(prompt)
        
        # Try to parse the result as JSON
        guideline_items = self._parse_json_result(result)
        
        # If parsing failed, try to extract JSON from text
        if not guideline_items:
            guideline_items = self._extract_json_from_text(result)
        
        # If extraction failed, try a different approach
        if not guideline_items:
            self.logger.warning("Failed to extract guideline items as JSON. Trying with structured text approach.")
            guideline_items = self._extract_items_structured(guideline_texts)
        
        self.logger.info(f"Extracted {len(guideline_items)} guideline items")
        
        return guideline_items
    
    def _extract_items_structured(self, guideline_texts: List[str]) -> List[Dict[str, Any]]:
        """
        ...
        REPORTING GUIDELINE TEXT:
        {text}
        """.format(text=combined_text[:100000])
    ...
    def generate_prompts(self, guideline_items: List[Dict[str, Any]]) -> Dict[str, str]:
        """
        ...
        """
        self.logger.info("Generating prompts for guideline items")
    ...
    def analyze_guideline_compliance(self, validation_results: Dict[str, Any], 
                                    guideline_items: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        """
        self.logger.info("Analyzing guideline compliance")
