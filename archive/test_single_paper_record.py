...
OUTPUT_PATH = "output"
PROMPT_FILE = "output/prompts/20250318_025844_openai_reasoner_RECORD_prompts.json"
...
def run_test_on_paper(paper_file, prompt_file):
    ...
    print(f"\nRunning test on {paper_file} with RECORD checklist...")
    subprocess.run(cmd)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run test on a single paper with RECORD checklist')
    ...
    if report_file:
        ...
        if result:
            # Print the result
            print("\n" + "="*80)
            print(f"RECORD VALIDATION RESULT FOR {args.paper_file}")
            print("="*80)
