...
# Script to copy RAG reports to reports_rag directory and run analysis

# Create reports_rag directory if it doesn't exist
mkdir -p output/reports_rag

# Copy all RAG reports to reports_rag directory
echo "Copying RAG reports to output/reports_rag directory..."
find output -name "*rag*report*_Li-Paper.json" -exec cp {} output/reports_rag/ \;

# Count the number of reports copied
num_reports=$(ls -1 output/reports_rag/ | wc -l)
echo "Copied $num_reports RAG reports to output/reports_rag directory"

# Create a modified version of run_analysis_only.sh to use reports_rag directory
cat > run_analysis_rag_only.sh << 'EOF'
#!/bin/bash
# run_analysis_rag_only.sh
# Script to run analysis on RAG reports

echo "Running modified Li-Paper analysis script on RAG reports"

# Create a temporary modified version of the analysis script
cat > run_li_paper_analysis_rag.py << 'EOFPY'
import os
import json
import pandas as pd
import numpy as np
...
# Run the modified analysis script
python run_li_paper_analysis_rag.py --reports_dir output/reports_rag --output_prefix "rag_"

echo "Analysis completed!"
echo "Visualizations have been saved as PNG files with 'rag_' prefix in the current directory."
echo "Summary statistics have been saved to rag_li_paper_summary_stats.csv"
echo "A detailed analysis report has been generated in li_paper_analysis_summary_rag.md"
EOF

# Make the script executable
chmod +x run_analysis_rag_only.sh

# Run the analysis
echo "Running analysis on RAG reports..."
./run_analysis_rag_only.sh
