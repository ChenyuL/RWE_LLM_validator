...
def process_papers(paper_list, delay=60):
    ...
    for paper in paper_list:
        ...
        if report_file:
            print(f"Found existing report for {paper}: {os.path.basename(report_file)}")
    ...
    print(f"\nFound {len(existing_results)} papers with existing reports")
    ...
    with open(log_file, 'w') as log:
        log.write(f"Processing remaining papers\n")
        log.write(f"Timestamp: {timestamp}\n\n")
        log.write(f"Found {len(existing_results)} papers with existing reports\n")
...
def generate_markdown_report(results, papers_with_issues, timestamp):
    ...
    report_content = f"# Validation Report for Li-Paper SOP Compliance\n\n"
    ...
    report_file = os.path.join(OUTPUT_PATH, f"{timestamp}_validation_report.md")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process papers that don\'t have report files yet')
