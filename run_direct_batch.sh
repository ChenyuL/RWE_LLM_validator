#!/bin/bash
# run_direct_batch.sh
# Script to run the direct extractor-validator (without RAG) on the sampled papers

# Make the Python scripts executable
chmod +x extractor_validator_direct.py
...
# Default values
DEFAULT_PROMPTS_FILE="/Users/chenyuli/LLMEvaluation/RWE_LLM_validator/output/prompts/20250325_excluded_Li-Paper_prompts.json"
...
DEFAULT_MAX_WORKERS=5 # Lower default for sequential processing of items
...
# Create a batch processing script for the direct extractor/validator
cat > run_direct_batch.py << 'EOF'
#!/usr/bin/env python
# run_direct_batch.py
# Script to run the direct extractor and validator (without RAG) on multiple papers
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("direct_batch.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("direct_batch")
...
def process_paper(paper_path, prompts_file, checklist, extractor_model, extractor_provider, validator_model, validator_provider):
    """
    Process a single paper using the direct extractor and validator.
    """
    paper_id = os.path.splitext(os.path.basename(paper_path))[0]
    logger.info(f"Processing paper: {paper_id}")
    
    cmd = [
        "python", "extractor_validator_direct.py",
    ...
    # Retry logic for subprocess execution
    retry_count = 0
        ...
        try:
            ...
            if "rate_limit" in error_message.lower() or "429" in error_message:
                if retry_count < MAX_RETRIES:
                    ...
                    logger.warning(f"Rate limit error for paper {paper_id}. Retrying in {actual_delay:.2f} seconds... (Attempt {retry_count}/{MAX_RETRIES})")
                    time.sleep(actual_delay)
                    
                    # Exponential backoff with max cap
                    retry_delay = min(retry_delay * 2, MAX_RETRY_DELAY)
                else:
                    logger.error(f"Max retries ({MAX_RETRIES}) exceeded for paper {paper_id}: {error_message}")
                    return {
                        "paper_id": paper_id,
                        "success": False,
                        "error": str(e),
...
def main():
    """
    Main function to run the direct extractor and validator on multiple papers.
    """
    parser = argparse.ArgumentParser(description='Run direct extractor and validator (without RAG) on multiple papers')
    ...
    parser.add_argument('--validator-model', type=str, default='gpt-4o', help='Model to use for validation (default: gpt-4o)')
    parser.add_argument('--validator-provider', type=str, default='openai', help='Provider to use for validation (default: openai)')
    ...
    # Save results
    output_file = f"direct_batch_results_{time.strftime('%Y%m%d_%H%M%S')}.json"
...
chmod +x run_direct_batch.py

# Run the direct batch processing
echo "Running direct extractor-validator batch processing (without RAG)..."
python run_direct_batch.py \
    --papers "$PAPERS_FILE" \
    --prompts "$PROMPTS_FILE" \
    --checklist "$CHECKLIST" \
