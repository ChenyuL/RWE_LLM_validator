#!/usr/bin/env python
# run_direct_batch.py
# Script to run the direct extractor and validator on multiple papers
...
def process_paper(paper_path, prompts_file, checklist, extractor_model, extractor_provider, validator_model, validator_provider, max_text_length=None):
    """
    ...
    # Add max_text_length parameter if provided
    if max_text_length:
        cmd.extend(["--max-text-length", str(max_text_length)])
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Run direct extractor and validator on multiple papers')
    ...
    parser.add_argument('--output-dir', type=str, default='output/reports_direct', help='Directory to save reports (default: output/reports_direct)')
    parser.add_argument('--max-text-length', type=int, help='Maximum text length to send to models (default: 24000)')
    ...
    output_file = os.path.join(args.output_dir, f"{args.extractor_model}_{args.validator_model}_direct_batch_results_{timestamp}.json")
    ...
    # Run analysis on the reports if requested
    if successful > 0:
        print("\nTo analyze the results, run:")
        print(f"python run_li_paper_analysis_complete.py --reports_dir {args.output_dir} --output_dir {args.output_dir}")
